...
function NeavRaid_OnEvent(self, event, ...)
    if ( event == "ADDON_LOADED" ) then
        ...
        if ( name == "oUF_NeavRaid" ) then
            NeavRaid_RegisterDefaultSetting("showSolo", false)
            NeavRaid_RegisterDefaultSetting("showParty", true)
            NeavRaid_RegisterDefaultSetting("assistFrame", false)
            NeavRaid_RegisterDefaultSetting("sortByRole", true)
            NeavRaid_RegisterDefaultSetting("anchorToControls", false)
            NeavRaid_RegisterDefaultSetting("horizontalHealthBars", false)
            NeavRaid_RegisterDefaultSetting("powerBars", true)
            NeavRaid_RegisterDefaultSetting("horizontalPowerBars", false)

            NeavRaid_RegisterDefaultSetting("orientation", "VERTICAL")
            NeavRaid_RegisterDefaultSetting("initialAnchor", "TOPLEFT")

            NeavRaid_RegisterDefaultSetting("nameLength", 4)
            NeavRaid_RegisterDefaultSetting("frameWidth", 48)
            NeavRaid_RegisterDefaultSetting("frameHeight", 46)
            NeavRaid_RegisterDefaultSetting("frameScale", 1.2)

            NeavRaid_RegisterDefaultSetting("indicatorSize", 7)
            NeavRaid_RegisterDefaultSetting("debuffSize", 22)
...
do
    indicatorList = {
        ...
        PRIEST = {
            ...
            {152118, "RIGHT", {0, 1, 0}},           	-- Clarity of Will
            {208065, "BOTTOMRIGHT", {0.5, 0.5, 1}}, 	-- Light of T"uure
            {41635, "TOPRIGHT", {1, 0, 0}},         	-- Prayer of Mending
            {139, "RIGHT", {0, 1, 0}},              	-- Renew
            {242622, "TOPLEFT", {1, 0, 0 }},        	-- The Deceiver"s Grand Design
            {253287, "TOP", {1, 1, 0}},         		-- Highfather's Timekeeping
        },
        SHAMAN = {
...
local function CreateIndicators(self, unit)
    ...
    self.AuraWatch = CreateFrame("Frame", "$parent_AuraWatch", self)
    ...
    if ( buffs ) then
        for key, spell in pairs(buffs) do
            ...
            local icon = CreateFrame("Frame", "$parent_Spell_"..spell[1], self.AuraWatch)
            ...
            local cd = CreateFrame("Cooldown", "$parent_CD", icon, "CooldownFrameTemplate")
            cd:SetAllPoints(icon)
            icon.cd = cd

                -- Indicator

            local tex = icon:CreateTexture("$parent_Texture", "OVERLAY")
            ...
            if ( not icon.hideCount ) then
                local count = icon:CreateFontString("$parent_Count", "OVERLAY")
...
local function CreateRaidLayout(self, unit)
    ...
    self.Health:SetAttribute("name", "$parent_HealthBarTex")
    ...
    self.Health.bg = self.Health:CreateTexture("$parent_Background", "BACKGROUND")
    ...
    self.Health.Value = self.Health:CreateFontString("$parent_Health")
    ...
    self.Name = self.Health:CreateFontString("$parent_Name")
    ...
    if ( nRaidDB.powerBars ) then
        self.Power = CreateFrame("StatusBar", "$parent_Power", self)
        self.Power:SetAttribute("name", "$parent_Power")
        ...
        self.Power.bg = self.Power:CreateTexture("$parent_PowerBG", "BACKGROUND")
    ...
    local myBar = CreateFrame("StatusBar", "$parent_MyHealthBar", self)
    myBar:SetAttribute("name", "$parent_MyHealthBar")
    ...
    local otherBar = CreateFrame("StatusBar", "$parent_OtherHealthBar", self)
    otherBar:SetAttribute("name", "$parent_OtherHealthBar")
    ...
    absorbBar.Overlay = absorbBar:CreateTexture("$parent_AbsorbBar_Overlay", "OVERLAY", "TotalAbsorbBarOverlayTemplate", -1)
    absorbBar.Overlay:SetAllPoints(absorbBar:GetStatusBarTexture())

    local healAbsorbBar = CreateFrame("StatusBar", "$parent_HealAbsorbBar", self)
    ...
    self.NotHere = self.Health:CreateFontString("$parent_StatusText", "OVERLAY")
    ...
    self.Mouseover = self.Health:CreateTexture("$parent_Darklight", "OVERLAY")
    ...
    self.ThreatIndicator = CreateFrame("Frame", "$parent_ThreatGlow", self)
    ...
    self.LeaderIndicator = self.Health:CreateTexture("$parent_LeaderIcon", "OVERLAY", nil, 7)
    self.LeaderIndicator:SetSize(12, 12)
    self.LeaderIndicator:SetPoint("LEFT", self.Health, "TOPLEFT", 1, 2)

        -- Assist Icon

    self.AssistantIndicator = self.Health:CreateTexture("$parent_AssistIcon", "OVERLAY", nil, 7)
    self.AssistantIndicator:SetSize(12, 12)
    self.AssistantIndicator:SetPoint("LEFT", self.Health, "TOPLEFT", 1, 2)

        -- Raid Target Indicator

    self.RaidTargetIndicator = self.Health:CreateTexture("$parent_RaidTargetIcon", "OVERLAY", nil, 7)
    self.RaidTargetIndicator:SetSize(16, 16)
    self.RaidTargetIndicator:SetPoint("CENTER", self, "TOP")

        -- Readycheck icons

    self.ReadyCheckIndicator = self.Health:CreateTexture("$parent_ReadyCheckIcon", "OVERLAY", nil, 7)
    ...
    self.FreebAuras = CreateFrame("Frame", "$parent_FreebAuras", self)
    ...
    self.GroupRoleIndicator = self.Health:CreateTexture("$parent_RoleIcon", "OVERLAY", nil, 6)
    self.GroupRoleIndicator:SetSize(12, 12)
    self.GroupRoleIndicator:SetPoint("BOTTOM", self.Health, 0, -6)

        -- Resurrect Indicator

    self.ResurrectIndicator = self.Health:CreateTexture("$parent_ResIcon", "OVERLAY", nil, 7)
    ...
    self.TargetBorder = self.Health:CreateTexture("$parent_TargetBorder", "OVERLAY")
...
SlashCmdList["oUF_Neav_Raid"] = function(msg)
