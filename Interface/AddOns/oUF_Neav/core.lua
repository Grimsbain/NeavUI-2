...
local function CreateTab(self, text)
    ...
    for i = 1, 3 do
        self.T[i] = self:CreateTexture("$parent_TabPart"..i, "BACKGROUND")
    ...
    self.T[4] = self:CreateFontString("$parent_TabText", "OVERLAY")
...
local function CreateFocusButton(self)
    self.FTarget = CreateFrame("BUTTON", "$parent_FocusTab", self, "SecureActionButtonTemplate")
...
local function CreateUnitLayout(self, unit)
    ...
    self.Texture = self:CreateTexture("$parent_FrameTexture", "BORDER")
    ...
    self.Health = CreateFrame("StatusBar", "$parent_HealthBar", self)
    ...
    local myBar = CreateFrame("StatusBar", "$parent_MyHealthPredictionBar", self)
    ...
    local otherBar = CreateFrame("StatusBar", "$parent_OtherHealthPredictionBar", self)
    ...
    local absorbBar = CreateFrame("StatusBar", "$parent_TotalAbsorbBar", self)
    ...
    absorbBar.Overlay = absorbBar:CreateTexture("$parent _Overlay", "ARTWORK", "TotalAbsorbBarOverlayTemplate", 1)
    ...
    local overAbsorb = self.Health:CreateTexture("$parent_OverAbsorb", "OVERLAY")
    ...
    local overHealAbsorb = self.Health:CreateTexture("$parent_OverHealAbsorb", "OVERLAY")
    ...
    self.Health.Value = self:CreateFontString("$parent_HealthText", "OVERLAY")
    ...
    self.Power = CreateFrame("StatusBar", "$parent_PowerBar", self)
    ...
    if ( self.IsTargetFrame ) then
		...
		self.Power.Value = self:CreateFontString("$parent_PowerText", "OVERLAY")
    ...
    self.Name = self:CreateFontString("$parent_Name", "OVERLAY")
    ...
    if ( self.IsMainFrame ) then
        self.Level = self:CreateFontString("$parent_Level", "ARTWORK")
    ...
    self.Portrait = self.Health:CreateTexture("$parent_Portrait", "BACKGROUND")
    ...
    elseif ( self.IsTargetFrame ) then
        self.Portrait:SetSize(35, 35) -- 40,40
    ...
    if ( config.show.portraitTimer ) then
        self.PortraitTimer = CreateFrame("Frame", "$parent_PortraitTimer", self.Health)
		self.PortraitTimer:SetAllPoints(self.Portrait)

        -- self.PortraitTimer.Icon = self.PortraitTimer:CreateTexture("$parent_PortraitTimerIcon", "BACKGROUND")
        -- self.PortraitTimer.Icon:SetAllPoints(self.Portrait)

        -- self.PortraitTimer.Remaining = self.PortraitTimer:CreateFontString("$parent_PortraitTimerText", "OVERLAY")
        -- self.PortraitTimer.Remaining:SetPoint("CENTER", self.PortraitTimer.Icon)
        -- self.PortraitTimer.Remaining:SetFont(config.font.normal, (self.Portrait:GetWidth()/3.5), "OUTLINE")
        -- self.PortraitTimer.Remaining:SetTextColor(1, 1, 1)
    end

        -- PvP Icon

    self.PvPIndicator = self:CreateTexture("$parent_PvPIcon", "OVERLAY", nil, 7)
    -- self.PvPIndicator.Prestige = self:CreateTexture("$parent_PrestigeIcon", "ARTWORK", nil, 7)

    if ( unit == "player" ) then
        self.PvPIndicator:SetSize(40, 42)
		self.PvPIndicator:SetPoint("TOPLEFT", self.Texture, 18, -20)
        -- self.PvPIndicator.Prestige:SetSize(40, 42)
    ...
    elseif ( unit == "target" or unit == "focus" ) then
        ...
        -- self.PvPIndicator.Prestige:SetSize(40, 42)
        -- self.PvPIndicator.Prestige:SetPoint("TOPRIGHT", self.Texture, -16, -23)
    elseif ( self.IsPartyFrame ) then
    ...
    self.LeaderIndicator = self:CreateTexture("$parent_LeaderIcon", "ARTWORK")
    ...
    self.AssistantIndicator = self:CreateTexture("$parent_AssistIcon", "ARTWORK")
    ...
    self.RaidTargetIndicator = self:CreateTexture("$parent_RaidTargetIcon", "ARTWORK")
    ...
    if ( not self.IsTargetFrame ) then
        self.PhaseIndicator = self:CreateTexture("$parent_PhaseIcon", "OVERLAY")
    ...
    self.OfflineIcon = self:CreateTexture("$parent_OfflineIcon", "OVERLAY")
    ...
    if ( unit == "player" or self.IsPartyFrame ) then
        self.ReadyCheckIndicator = self:CreateTexture("$parent_ReadyCheckIcon", "OVERLAY", nil, 7)
    ...
    self.ThreatGlow = self:CreateTexture("$parent_ThreatGlow", "BACKGROUND")
    ...
    if ( self.IsPartyFrame or unit == "player" or unit == "target" ) then
        self.GroupRoleIndicator = self:CreateTexture("$parent_GroupRoleIcon", "ARTWORK")
    ...
    if ( unit == "player" ) then
        ...
        self.Name.Bg = self:CreateTexture("$parent_NameBG", "BACKGROUND")
        ...
        if ( config.units.player.showAFKTimer ) then
            self.NotHere = self:CreateFontString("$parent_NotHere", "OVERLAY")
        ...
        if (   playerClass == "DRUID"
            ...
            self.AdditionalPower = CreateFrame("StatusBar", "$parent_AdditionalPower", self)
            ...
            self.AdditionalPower:SetBackdropColor(0, 0, 0, 0.70) -- 0.55
            self.AdditionalPower.colorPower = true

            self.AdditionalPower.Value = self.AdditionalPower:CreateFontString("$parent_AdditionalPowerText", "OVERLAY")
            ...
            self.AdditionalPower.Texture = self.AdditionalPower:CreateTexture("$parent_AdditionalPowerTexture", "ARTWORK")
            ...
            self.PowerPredictionAlt = CreateFrame("StatusBar", "$parent_AltPowerPrediction", self.AdditionalPower)
        ...
        if ( self.PvPIndicator ) then
            self.PvPTimer = self:CreateFontString("$parent_PvPTimer", "OVERLAY")
        ...
        local LootSpecIndicator = self:CreateTexture("$parent_SpecIcon")
        ...
        LootSpecIndicator.Border = self:CreateTexture("$parent_SpecIconRing")
        ...
        self.RestingIndicator = self:CreateTexture("$parent_RestingIcon", "OVERLAY")
        self.RestingIndicator:SetPoint("TOPLEFT", self.Texture, 39, -50)
        self.RestingIndicator:SetSize(31, 31) --31,34

        self.RestingIndicator.Glow = self:CreateTexture("$parent_RestingIconGlow", "OVERLAY")
        ...
        self.CombatIndicator = self:CreateTexture("$parent_CombatIcon", "OVERLAY")
        ...
        self.CombatIndicator.Glow = self:CreateTexture("$parent_CombatIconGlow", "OVERLAY")
        ...
        self.StatusFlash = self:CreateTexture("$parent_StatusFlash", "ARTWORK")
    ...
    if ( unit == "pet" ) then
        ...
        if ( not config.units[ns.cUnit(unit)].disableAura ) then
            self.Debuffs = CreateFrame("Frame", "$parent_Debuffs", self)
    ...
    if ( unit == "target" or unit == "focus" ) then
        ...
        self.QuestIndicator = self:CreateTexture("$parent_QuestIcon", "OVERLAY")
    ...
    if ( unit == "target" ) then
        ...
        if ( config.units.target.showThreatValue ) then
            ...
            self.NumericalThreat.bg = self.NumericalThreat:CreateTexture("$parent_NumericalThreatBG", "ARTWORK")
            ...
            self.NumericalThreat.value = self.NumericalThreat:CreateFontString("$parent_NumericalThreatText", "OVERLAY", "GameFontHighlight")
            self.NumericalThreat.value:SetPoint("TOP", 1, -3)

            self.NumericalThreat.texture = self.NumericalThreat:CreateTexture("$parent_NumericalThreatTexture", "ARTWORK")
		...
		self.petBattleIcon = self:CreateTexture("$parent_PetBattleIcon", "ARTWORK")
    ...
    if ( unit == "focus" ) then
        ...
        self.FClose = CreateFrame("Button", "$parent_FClose", self, "SecureActionButtonTemplate")
